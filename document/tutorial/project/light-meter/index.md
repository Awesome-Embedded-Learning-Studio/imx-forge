...
它的两个 git commit 就是本系列的脊柱。你现在 `git log --oneline` 能看到,`0bb4252` 是桌面 Mock 阶段的全部,Windows 和 Linux 双平台都能跑、不接硬件;`be4fb4d` 是真机后端接入,翻一个 CMake 开关就去读你板子上那颗真实的 AP3216C。本系列的章节顺序,就是沿着这两刀走的。
