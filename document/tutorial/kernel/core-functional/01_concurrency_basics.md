...
我们现在面对的ARM SoC，双核、四核甚至八核都是标配。咱们手上这块i.MX6ULL反而是个例外——它是**单核**Cortex-A7。单核不代表没有并发：中断、抢占、软中断照样会把你的代码打断，一样能造出「幽灵Bug」。而多核SoC意味着更严酷的东西：真正的物理并行，意味着你的代码可能在Core 0上跑的同时，另一个线程在Core 1上跑。它们是**同时**执行的，不是轮流切换的。驱动代码必须按多核模型思考——同一份代码换个双核板子，单核思维里的「顺序」就不存在了。
...
| 特殊约束 | 内核按 SMP 配置编译（单 CPU），内核抢占启用 |

这个配置很重要，因为并发问题的表现形式会根据配置不同而变化。比如在单核非抢占内核上，某些竞争条件可能永远不会出现；但在我们的抢占式内核上，它们就是家常便饭（单核上来自中断与抢占，多核上还要加上真并行）。
