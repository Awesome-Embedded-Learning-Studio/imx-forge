...
并发与同步是 Linux 内核里最容易踩坑、也最难调试的部分。一段在不开抢占时永远正确的代码，换到 i.MX6ULL 的单核 Cortex-A7 + 抢占环境下（中断、抢占、软中断随时打断你），就可能因为时序的细微差异而偶发崩溃——这类「幽灵 Bug」往往调一整天都复现不了；换到多核 SoC 上，还要再加上真正的物理并行，情况只会更严酷。
