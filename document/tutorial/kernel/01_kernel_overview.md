...
我当时的问题是：我只是想在自己的i.MX6ULL板子上跑一个Linux系统，我需要理解哪些东西？主线内核和厂商BSP有什么区别？为什么有些教程说用5.x版本，有些说用4.x？内核源码这堆目录都是干嘛的？
...
BSP（Board Support Package）内核是厂商基于主线内核修改的版本。以NXP为例，他们维护的BSP仓库包含了i.MX系列处理器的所有专用驱动和补丁。比如NXP的GALCORE GPU驱动、专门的电源管理代码、i.MX特有的外设驱动等等。这些东西在主线内核里要么没有，要么版本落后。

对于i.MX6ULL这样的芯片，行业里的常见做法是生产用厂商BSP保功能、学习用主线内核看原理。IMX-Forge的立场更彻底：**全线单轨主线内核**——我们已经在主线 7.1 上把 LCD、触摸、双网口、音频这些板级功能全部跑通了，BSP 不再是必需品。跟着本教程走，你接触到的源码、配置、设备树全部来自主线。

说个好消息：主线内核其实很潮。全志的SDK或者是RK的SDK基本上不是5.4就是6.1，而咱们直接用主线 7.1（项目早期走过 NXP BSP 6.12.x 的路线，2026 年 9 月完成单轨化，BSP 轨道已退役）。
...
- 成本低。开发板便宜，适合个人学习和实验
- 性能适中。单核 Cortex-A7，最高792MHz，足够运行Linux和常见应用
...
好了，现在假设你已经下载了主线内核源码（IMX-Forge项目里在`third_party/linux_mainline`），第一次看到这个庞大的目录树，心里发虚。别慌，我们来逐个目录看看。
