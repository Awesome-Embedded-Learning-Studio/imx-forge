...
GDB（GNU Debugger）解决了这些问题。它让你在程序运行时暂停执行、检查变量、单步推进、查看调用栈——甚至可以在不重新编译的情况下改变执行流程。对于嵌入式开发来说，GDB 的远程调试功能（`gdbserver`）是调试开发板上程序的标配方案——[gdbserver 远程调试全链](../../debug/01_gdbserver_remote_debug.md)已把这条链完整展开。
...
Core dump 则是这个能力的延伸：程序在生产环境崩了，你把 core 文件拷回来，在本机用 GDB 分析，等于把事故现场搬回了实验室。对于嵌入式开发来说，这意味着开发板上的崩溃你可以在开发机上远程调试——崩溃现场怎么抓回来（coredump）见 [strace、日志与 coredump](../../debug/02_strace_log_coredump.md)。
