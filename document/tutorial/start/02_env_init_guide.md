...
::: info 您将学到
- env-init.sh 在整条构建链里的位置，以及它与上一篇装的交叉工具链怎么分工
- 从 --help 到按 stage 检查的完整用法，附本机实跑输出
- 四张 stage 依赖表的内容与来历，包括 Buildroot 为什么不在表里
- 缺包时的交互式安装流程：/dev/tty 的读取机制与 y/n 两个分支
- 五个可导入的检查函数，以及 build-uboot.sh 这类脚本怎么复用它们
:::
