...
::: info 您将学到
- 为什么嵌入式 Linux 必须用交叉编译工具链，以及 2026 年了为什么不该再用 GCC 7
- Standalone Toolchain(独立工具链)是什么，为什么优先选它而不是厂商 SDK 里那份
- 下载、解压、安装到 /opt、配置 PATH、验证，五步完整走通，验证输出全部本机实测
- PATH 少写 bin、写错 shell 配置文件这类高频翻车点，怎么几分钟内自查出来
:::
