...
// 跳转加载指示（移植自姊妹项目 TAMCPP，issue #167）：SPA 点链接后 vitepress
// 要先去拉目标页的 JS chunk，慢网/冷缓存时旧页面会「死寂几秒 → 内容瞬间换掉
// + 弹回顶部」，没有任何加载反馈。两级反馈：
//   220ms 起 —— 左下角「加载中」小卡（目标页模块已缓存时 loadPage 是微任务
//               级的，阈值内不弹，弹了反而闪）
//   3s 起   —— 整站蒙半透明浮层，说明站点部署在 GitHub Pages 上可能有访问延迟
  ...
  <Transition name="nav-veil">
    <div v-if="veilVisible" class="nav-veil" role="status" aria-live="polite">
      <div class="nav-veil__box">
        ...
        <p class="nav-veil__sub">网站部署在 GitHub Pages 上，可能访问会有延迟</p>
