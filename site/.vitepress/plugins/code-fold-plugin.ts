 ...
 *   - resolvePlugins(md) 在 VitePress 把 Shiki/复制按钮/行号/preWrapper 全部接好
 ...
 *
 * 阈值默认 20(依据全站实测:20 行以内的中等块保持原样、不误伤阅读,超过的多为演示性
 * 大段输出或完整工程才折叠)。改 project.config.ts 的 readingDefaults.codeFoldLines 即可全局调档。
 */
export const codeFoldPlugin = (lines = 20): PluginSimple => (md: MarkdownIt) => {
  const FOLD_THRESHOLD = lines
